...
echo "run-tcp-one-all.sh currently running on ${myIP}"
...
while IFS='' read -r line || [ -n "$line" ]; do
    ...
    ssh $line -tt -o StrictHostKeyChecking=no "iperf -c ${myIP} -p ${currPort} -t $1 > /dev/null" &
done < "hosts"
sleep $1
...
pgrep iperf | xargs kill -9
echo "run-tcp-one-all.sh finished running on ${myIP}"
